...
CHROMA_PATH = r"chroma_db\chroma_semantic" # Path to the directory where the database will be saved and name of the database
...
def split_text(documents: list[Document]):
    text_splitter = SemanticChunker(GoogleGenerativeAIEmbeddings(model="models/text-embedding-004")) # Embedding model 
    chunks = text_splitter.split_documents(documents)
    print(f"Split {len(documents)} documents into {len(chunks)} chunks.")

    # To see the content of a random chunk(10) and check if there is a problem with the text splitting
    document = chunks[10]
